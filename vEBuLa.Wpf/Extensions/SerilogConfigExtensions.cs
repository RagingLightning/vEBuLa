...
internal static class SerilogConfigExtensions {
  ...
  public static LoggerConfiguration ApplyDestructuringRules(this LoggerConfiguration config) {
    return config
      ...
      .Destructure.ByTransforming<SetupScreenVM>(vm => new {UsingCustom = vm.UsingCustom, CustomRoute = vm.CustomRoute, PredefinedRoute = vm.SelectedRoute, Departure = vm.ServiceStart });
  }
}
